...
class PluginTest:
    ...
    def __init__(self, serializer_path: str, discover_path: Path):
        ...
        self._expected_string: dict = {"hello": "world"}
    ...
    def _validate_plugin_exists(self) -> Tuple[int, str]:
        ...
        error_count = 0
        error_message = ""
        if PluginManager.is_plugin_exists(PluginType.SERIALIZER, "jsonserializer"):
            # Serializer is found
            pass
        else:
    ...
    def _validate_metadata(self) -> Tuple[int, str]:
        ...
        if (
            metadata.name == "jsonserializer"
            and metadata.description
            == "jsonserializer supports deserializing json data"
